    ...
    if (!artefactUrl) {
        ...
        if (videos.length === 0) {
            ...
            const videoLinks = await Promise.all(videos.map((videoObject) => {
                ...
                return `<${videosURL}/${videosDir}/${path.relative(videosDir, videoObject)}|Video:- ${trimmedVideoFilename}>\n`;
            }));
    ...
    if (!artefactUrl) {
        ...
        if (screenshots.length === 0) {
            ...
            const screenshotLinks = await Promise.all(screenshots.map((screenshotObject) => {
                ...
                return `<${screenshotURL}/${screenshotDir}/${path.relative(screenshotDir, screenshotObject)}|Screenshot:- ${trimmedScreenshotFilename}>\n`;
            }));
...
const resolveCIProvider = async (ciProvider) => {
    let { CI_SHA1, CI_BRANCH, CI_USERNAME, CI_BUILD_URL, CI_BUILD_NUM, CI_PULL_REQUEST, CI_PROJECT_REPONAME, CI_PROJECT_USERNAME, JOB_NAME, http_proxy, CIRCLE_PROJECT_ID, } = process.env;
